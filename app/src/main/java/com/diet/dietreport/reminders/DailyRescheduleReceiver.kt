...
/**
 * Fires once daily (at wake time) to schedule that day's reminder slots.
 * Each invocation also schedules the next day's trigger, creating a
 * self-perpetuating daily chain.
 */
class DailyRescheduleReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        Log.d(TAG, "Daily reschedule alarm fired — scheduling today's reminders")
        ...
        CoroutineScope(Dispatchers.IO + SupervisorJob()).launch {
            try {
                ...
                Log.e(TAG, "Failed to reschedule daily reminders", e)
            } finally {
